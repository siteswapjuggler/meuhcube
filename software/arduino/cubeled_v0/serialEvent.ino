...
#define MOVE_XYZ    0x21
#define MODU_XYZ    0x22
#define ROTA_XYZ    0x23
#define MIRO_XYZ    0x24
                      ...
                      switch (command) {
                        ...
                        case SET_LINE: 
                          if (length==4) {
                            ...
                            switch (p) {
                              ...
                              case 1 :
                                for (int line = 0; line < SIZE; line++) {
                                  bitWrite(cubeVoxels[line][c1],c2,bitRead(v,line));                                
                                }
                                break;
                              case 2 :
                                for (int column = 0; column < SIZE; column++) {
                                  bitWrite(cubeVoxels[c1][column],c2,bitRead(v,column));                                
                                }
                                break;
                            }
                          }
                        ...
                        case SET_CUBE: 
                          if (length==49) {
                            ...
                            for (byte i = 0; i < 49; i++) {
                              byte line = i % SIZE;
                              byte column = i / SIZE;
                              cubeVoxels[line][column] = buffer[i]&B1111111; 
                            }
                          }
                          break;
                        ...
                        case REV_ALL:
                          if (length==0) {
                            for (int line = 0; line < SIZE; line++) {
                              for (int column = 0; column < SIZE; column++) {
                                cubeVoxels[line][column] = (byte)~cubeVoxels[line][column];
                              }
                            }
                          }
                          break;
                        ...
                        case REV_PIXEL:
                          if (length==4) {
                            ...
                            y = buffer[1] < SIZE ? buffer[1] : SIZE-1;
                            z = buffer[2] < SIZE ? buffer[2] : SIZE-1;
                            v = bitRead(cubeVoxels[x][y],y)^(buffer[3] > 0);
                            bitWrite(cubeVoxels[x][z],y,v);
                          }
                          break;
                        ...
                        case REV_LINE: 
                          if (length==4) {
                            ...
                            switch (p) {
                              case 0 :
                                cubeVoxels[c1][c2] = cubeVoxels[c1][c2]^v;
                                break;
                              case 1 :
                                for (int line = 0; line < SIZE; line++) {
                                  t = bitRead(cubeVoxels[line][c1],c2) ^ bitRead(v,line);
                                  bitWrite(cubeVoxels[line][c1],c2,t);                                
                                }
                                break;
                              case 2 :
                                for (int column = 0; column < SIZE; column++) {
                                  t = bitRead(cubeVoxels[c1][column],c2) ^ bitRead(v,column);
                                  bitWrite(cubeVoxels[c1][column],c2,t);                                
                                }
                                break;
                            }
                          }
                        ...
                        case REV_CUBE: 
                          if (length==49) {
                            ...
                            for (byte i = 0; i < 49; i++) {
                              byte line = i % SIZE;
                              byte column = i / SIZE;
                              cubeVoxels[line][column] = cubeVoxels[line][column]^(buffer[i]&B1111111); 
                            }
                          }
                          break;
